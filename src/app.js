...
function setupSplitters() {
  ...
  $$('.splitter', shell).forEach(splitter => {
    ...
    splitter.addEventListener('dblclick', () => {
      state.resize.files = 250;
      state.resize.editor = 520;
      shell.style.setProperty('--files-width', '250px');
      shell.style.setProperty('--editor-width', '520px');
    });
  });
}
...
async function loadProject(item) {
  ...
  await Promise.all(list.map(async relative => {
    ...
    try {
      const response = await fetch(url, { cache: 'no-store' });
      if (!response.ok) {
        state.files.set(relative, { binary: isBinary(relative), url, error: response.status });
        return;
      }
...
function onEdit() {
  ...
  state.saveTimer = setTimeout(() => saveCurrent(false), 450);
...
function updatePreview() {
  ...
  cleanupPreview();
  state.previewObjectUrl = URL.createObjectURL(new Blob([html], { type: 'text/html' }));
  const frame = $('#previewFrame');
  if (frame) frame.src = state.previewObjectUrl;
