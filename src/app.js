...
function renderFileList(editable) {
  ...
  el.innerHTML = sorted.map(rel => `<button type="button" class="file-btn" data-file="${escapeAttr(rel)}"><span>${fileIcon(rel)}</span><span>${escapeHtml(rel)}</span></button>`).join('');
  el.querySelectorAll('.file-btn').forEach(btn => btn.onclick = () => selectFile(btn.dataset.file));
}
