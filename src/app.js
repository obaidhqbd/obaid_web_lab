...
async function boot() {
  try {
    const initialClasses = readInitialJSON('initial-classes');
    const initialConfig = readInitialJSON('initial-config');
    const [classes, config] = await Promise.all([
      Array.isArray(initialClasses) ? initialClasses : getJSON('classes.json'),
      initialConfig && typeof initialConfig === 'object' ? initialConfig : getJSON('site.config.json').catch(() => ({}))
    ]);
