...
const state = {
  ...
  saveTimer: null, previewTimer: null, searchTimer: null,
  currentFilter: 'All', cmPromise: null, zipPromise: null,
  resize: { files: 250, editor: 520 }, loadToken: 0
};
...
function applyBrand() {
  ...
  if (portfolio && brand.portfolio) {
    portfolio.href = brand.portfolio;
    portfolio.textContent = brand.portfolioLabel || 'View my portfolio ↗';
  }
...
function workspaceTemplate(item) {
  ...
  return `<div class="editor-shell" id="editorShell">
    ...
    </aside><div class="splitter" data-split="files" role="separator" aria-orientation="vertical" tabindex="0" aria-label="Resize project files panel"></div>
    ...
    <div class="splitter" data-split="editor" role="separator" aria-orientation="vertical" tabindex="0" aria-label="Resize code editor panel"></div>
...
function setDefaultWorkspaceSize() {
  ...
  shell.style.setProperty('--files-width', `${state.resize.files}px`); shell.style.setProperty('--editor-width', `${state.resize.editor}px`);
}
function startSplitDrag(event) {
  ...
  const move = e => { const x = e.clientX - rect.left; if (kind === 'files') { state.resize.files = Math.round(Math.max(220, Math.min(360,x))); shell.style.setProperty('--files-width', `${state.resize.files}px`); return; } const available = rect.width - state.resize.files - 12; state.resize.editor = Math.round(Math.max(380, Math.min(available - 380, x - state.resize.files - 8))); shell.style.setProperty('--editor-width', `${state.resize.editor}px`); };
...
function createCodeMirrorEditor(cm,host,relative,value){
  ...
  state.editorView=new cm.view.EditorView({state:cm.state.EditorState.create({doc:value,extensions}),parent:host}); window.__cm={undo:cm.commands.undo,redo:cm.commands.redo};
}
function createFallbackEditor(host,value){const textarea=document.createElement('textarea'); textarea.className='fallback-editor'; textarea.value=value; textarea.spellcheck=false; textarea.wrap='off'; textarea.addEventListener('input',onEdit); host.innerHTML=''; host.appendChild(textarea); state.editorView={fallback:true,getValue:()=>textarea.value,setValue:n=>textarea.value=n};}
function cleanupEditor(){state.editorView?.destroy?.();state.editorView=null;}
...
function onEdit(){syncEditorValue();$('#saveStatus')&&($('#saveStatus').textContent='Unsaved local edit');clearTimeout(state.saveTimer);clearTimeout(state.previewTimer);const targetId=state.activeClass?.id||state.activeClass?.slug;state.saveTimer=setTimeout(()=>{if((state.activeClass?.id||state.activeClass?.slug)===targetId)saveCurrent(false);},500);$('#previewState')&&($('#previewState').textContent='UPDATING…');state.previewTimer=setTimeout(()=>{if((state.activeClass?.id||state.activeClass?.slug)===targetId)updatePreview();},350);}
...
function updatePreview(){if(!state.activeClass)return;syncEditorValue();const htmlFile=state.activeClass.entry&&state.files.has(state.activeClass.entry)?state.activeClass.entry:[...state.files.keys()].find(f=>(/(^|\/)index\.html?$/i).test(f));if(!htmlFile)return;const source=state.files.get(htmlFile)?.current??state.files.get(htmlFile)?.original??'';const html=injectProject(source,htmlFile);const frame=$('#previewFrame');if(!frame)return;frame.srcdoc=html;$('#previewState').textContent='LIVE';}
...
document.addEventListener('pointermove',event=>{const card=event.target.closest('.class-card');if(!card)return;requestAnimationFrame(()=>{const r=card.getBoundingClientRect();card.style.setProperty('--mx',`${event.clientX-r.left}px`);card.style.setProperty('--my',`${event.clientY-r.top}px`);});},{passive:true});
...
async function boot(){try{const [classes,config]=await Promise.all([getJSON('classes.json'),getJSON('site.config.json').catch(()=>({}))]);if(!Array.isArray(classes))throw new Error('classes.json must contain an array.');state.classes=classes;state.siteConfig=config||{};applyBrand();buildFilters();renderClasses();$('#search')?.addEventListener('input',()=>{clearTimeout(state.searchTimer);state.searchTimer=setTimeout(renderClasses,60);});const match=location.hash.match(/^#workspace\/(.+)$/);if(match)openClass(decodeURIComponent(match[1]),false);}catch(error){$('#classGrid').innerHTML=`<div class="empty"><strong>Project library unavailable.</strong><br>${escapeHtml(error.message)}</div>`;}}
