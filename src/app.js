...
const state = {
  ...
  currentFilter: 'All', cmPromise: null, zipPromise: null, formatPromise: null, formatting: false,
...
const MODULES = {
  ...
  prettier: 'https://esm.sh/prettier@3.6.2/standalone',
  prettierHtml: 'https://esm.sh/prettier@3.6.2/plugins/html',
  prettierCss: 'https://esm.sh/prettier@3.6.2/plugins/postcss',
  prettierBabel: 'https://esm.sh/prettier@3.6.2/plugins/babel',
  prettierEstree: 'https://esm.sh/prettier@3.6.2/plugins/estree' 
};
...
function applyBrand() {
  ...
  const mentorName = brand.mentorName || 'Mohammed Obaidul Hoque';
  const mentorRole = brand.mentorRole || 'Web Development Mentor';
  const portfolio = brand.portfolio || brand.website || 'https://obaidhqbd.github.io/next-gen-portfulio/';
  ...
  $('#heroMentorName') && ($('#heroMentorName').textContent = mentorName);
  $('#heroMentorRole') && ($('#heroMentorRole').textContent = mentorRole);
  $('#heroPortfolioLink') && ($('#heroPortfolioLink').href = portfolio);
  $('#portfolioLink') && ($('#portfolioLink').href = portfolio);
  const avatar = String(brand.avatar || '').trim();
  setMentorAvatar(avatar);
  document.title = `${name} · ${brand.role || 'Learning Lab'}`;
}

function setMentorAvatar(avatar) {
  const targets = [
    [$('#heroMentorPhoto'), $('#heroMentorFallback')],
    [$('#mentorAvatar'), $('#mentorAvatarFallback')]
  ];
  for (const [img, fallback] of targets) {
    if (!img || !fallback) continue;
    img.hidden = !avatar;
    fallback.hidden = Boolean(avatar);
    if (!avatar) { img.removeAttribute('src'); continue; }
...
function cleanupEditor(){state.editorResizeObserver?.disconnect?.();state.editorResizeObserver=null;state.editorView?.destroy?.();state.editorView=null;state.formatting=false;}
...
function onEdit(){
  ...
  state.saveTimer=setTimeout(()=>{
    if((state.activeClass?.id||state.activeClass?.slug)===targetId) void saveCurrent(false);
  },650);
...
function formatterParser(relative){
  if(/\.html?$/i.test(relative)) return 'html';
  if(/\.css$/i.test(relative)) return 'css';
  if(/\.ts$/i.test(relative)) return 'babel-ts';
  if(/\.(m?js)$/i.test(relative)) return 'babel';
  return null;
}

async function loadFormatter(){
  if(!state.formatPromise){
    state.formatPromise=Promise.all([
      import(MODULES.prettier),
      import(MODULES.prettierHtml),
      import(MODULES.prettierCss),
      import(MODULES.prettierBabel),
      import(MODULES.prettierEstree)
    ]).then(([core,html,css,babel,estree])=>({
      prettier:core.default||core,
      plugins:[html.default||html,css.default||css,babel.default||babel,estree.default||estree]
    })).catch(error=>{ state.formatPromise=null; throw error; });
  }
  return state.formatPromise;
}

function replaceEditorText(nextValue){
  if(!state.editorView || !state.activeFile) return false;
  const current=state.files.get(state.activeFile)?.current ?? '';
  if(current===nextValue) return false;
  state.formatting=true;
  try{
    if(state.editorView.state && typeof state.editorView.dispatch==='function'){
      state.editorView.dispatch({changes:{from:0,to:state.editorView.state.doc.length,insert:nextValue}});
    }else{
      state.editorView.setValue?.(nextValue);
    }
    const item=state.files.get(state.activeFile);
    if(item) item.current=nextValue;
    return true;
  }finally{
...
async function formatActiveFile(){
  if(state.formatting || !state.activeFile) return false;
  const parser=formatterParser(state.activeFile);
  if(!parser) return false;
  const item=state.files.get(state.activeFile);
  if(!item || item.binary || item.error) return false;
  const source=item.current??item.original??'';
  if(!source.trim()) return false;
  const saveBefore=state.activeFile;
  try{
    $('#saveStatus')&&($('#saveStatus').textContent='Formatting…');
    const {prettier,plugins}=await loadFormatter();
    if(saveBefore!==state.activeFile) return false;
    ...
    if(saveBefore!==state.activeFile) return false;
    return replaceEditorText(formatted);
  }catch(error){
    console.warn('Auto-format skipped:',error);
    return false;
  }
}
...
async function saveCurrent(manual=false){
  ...
  syncEditorValue();
  if(!state.formatting){ await formatActiveFile(); syncEditorValue(); }
  const payload={version:5,updatedAt:new Date().toISOString(),files:{}};
  ...
  try{
    ...
    $('#saveStatus')&&($('#saveStatus').textContent=manual?'Saved + formatted':'Autosaved + formatted');
    renderClasses();
  }catch{
    $('#saveStatus')&&($('#saveStatus').textContent='Local save unavailable');
    showToast('Local browser storage is full. Export your work as ZIP.');
  }
}
