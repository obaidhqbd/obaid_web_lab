...
function readClassMetadata(classDirName, files) {
  const dir = classDirName === '.' ? classesDir : path.join(classesDir, classDirName);
...
function addClass(sourceDir, slugHint) {
  const files = walkFiles(sourceDir);
  if (!files.some(f => /\.html?$/i.test(f))) return false;
  const info = readClassMetadata(slugHint, files);
  const requestedId = String(info.id || slugHint);
  if (seenIds.has(requestedId)) {
    throw new Error(`Duplicate class id: ${requestedId}`);
  }
  seenIds.add(requestedId);
  classesData.push(info);
  const destinationSlug = info.slug || slugHint;
  fs.cpSync(sourceDir, path.join(distClassesDir, destinationSlug), { recursive: true });
  return true;
}

// Support the original recommended structure: classes/<slug>/*
// Also support a single-class root layout: classes/index.html + classes/class.json + assets.
const directRootFiles = fs.readdirSync(classesDir, { withFileTypes: true })
  .filter(entry => entry.isFile() && entry.name !== '.DS_Store')
  .map(entry => entry.name);
const rootClassFiles = walkFiles(classesDir);
const rootHasHtml = directRootFiles.some(f => /\.html?$/i.test(f));
const childDirs = fs.readdirSync(classesDir, { withFileTypes: true })
  .filter(entry => entry.isDirectory() && !entry.name.startsWith('.'))
  .map(entry => entry.name);

if (rootHasHtml) {
  const rootInfo = readClassMetadata('.', directRootFiles);
  const slug = rootInfo.id || humanize(rootInfo.title).toLowerCase().replace(/\s+/g, '-');
  const files = rootClassFiles;
  if (files.some(f => /\.html?$/i.test(f))) {
    if (seenIds.has(rootInfo.id)) throw new Error(`Duplicate class id: ${rootInfo.id}`);
    seenIds.add(rootInfo.id);
    classesData.push({ ...rootInfo, slug });
    const rootDestination = path.join(distClassesDir, slug);
    fs.mkdirSync(rootDestination, { recursive: true });
    for (const relative of directRootFiles) {
      fs.copyFileSync(path.join(classesDir, relative), path.join(rootDestination, relative));
    }
  }
}

for (const child of childDirs) {
  const classDir = path.join(classesDir, child);
  const files = walkFiles(classDir);
  if (!files.some(f => /\.html?$/i.test(f))) {
    console.warn(`Skipping ${child}: no HTML entry/source file found.`);
    continue;
  }
  addClass(classDir, child);
}
...
fs.writeFileSync(path.join(distDir, 'robots.txt'), `User-agent: *\nAllow: /\nSitemap: https://obaidhqbd.github.io/obaid_web_lab/sitemap.xml\n`);
const siteOrigin = 'https://obaidhqbd.github.io/obaid_web_lab/';
const sitemapUrls = [siteOrigin, ...classesData.map(c => `${siteOrigin}#workspace/${encodeURIComponent(c.id || c.slug)}`)];
fs.writeFileSync(path.join(distDir, 'sitemap.xml'), `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${sitemapUrls.map(loc => `<url><loc>${loc}</loc></url>`).join('')}</urlset>\n`);
