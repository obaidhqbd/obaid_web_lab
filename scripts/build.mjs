...
if (fs.existsSync(builtIndexPath)) {
  ...
  if (fs.existsSync(configPath)) {
    try { configData = JSON.parse(fs.readFileSync(configPath, 'utf8')) || {}; } catch { configData = {}; }
  }
  let html = fs.readFileSync(builtIndexPath, 'utf8');
  const safe = value => JSON.stringify(value).replace(/</g, '\u003c');
  const bootstrap = `\n<script type="application/json" id="initial-classes">${safe(classesData)}</script>\n<script type="application/json" id="initial-config">${safe(configData)}</script>\n`;
  html = html.replace('</body>', `${bootstrap}</body>`);
  fs.writeFileSync(builtIndexPath, html);
}
